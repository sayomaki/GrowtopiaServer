...
unsigned long _byteswap_ulong(__uint64_t val) { return __builtin_bswap32(val);}
...
void SendPacketRaw(int a1, void *packetData, size_t packetDataSize, void *a4, ENetPeer* peer, int packetFlag)
{
	...
	void sendWorld(ENetPeer* peer, WorldInfo* worldInfo)
	{
		...
		__int16_t nameLen = worldName.length();
...
{
	cout << "Growtopia private server (c) Growtopia Noobs, willi123yao" << endl;
	enet_initialize();
	...
	*p = 5;*/
	...
	while (enet_host_service(server, &event, 1000) > 0)
	{
		...
		{
			if(peerReceiveHandler(L, peer, event.packet)) continue;
